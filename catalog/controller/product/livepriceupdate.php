...
class ControllerProductLivepriceupdate extends Controller {
	...
	public function index() {
		...
		$cache_key = 'live_price_update'. md5($product_id . $quantity. $options_hash . $this->config->get('config_currency') . $this->session->data['language']);
		...
		if (!$this->use_cache || (!$json = $this->cache->get($cache_key))) {
			...
			if ($product_info) {
				...
				if ($data['price']) {
					$json['new_price']['price'] = $this->currency->format($this->tax->calculate(($data['price'] + $options_makeup), $product_info['tax_class_id'], $this->config->get('config_tax')) * $quantity, $this->session->data['currency']);
				...
				if ($data['special']) {
					$json['new_price']['special'] = $this->currency->format($this->tax->calculate(($data['special'] + $options_makeup), $product_info['tax_class_id'], $this->config->get('config_tax')) * $quantity, $this->session->data['currency']);
				} else {
					$json['new_price']['special'] = false;
				}
		
				if ($this->config->get('config_tax')) {
					$json['new_price']['tax'] = $this->currency->format(((float)$product_info['special'] ? ($product_info['special'] + $options_makeup) : ($product_info['price'] + $options_makeup)) * $quantity, $this->session->data['currency'] );
				} else {
